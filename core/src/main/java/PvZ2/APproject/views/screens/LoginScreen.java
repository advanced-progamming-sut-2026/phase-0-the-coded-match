...
public class LoginScreen extends BaseScreen {
    ...
    public void show() {
        ...
        backgroundImage.setFillParent(true);stage.addActor(backgroundImage);wrapper = new BorderedTable();
        ...
        wrapper.add(stayLoggedInCheckBox).padBottom(10).row();wrapper.pack();
        wrapper.setPosition((VIRTUAL_WIDTH - wrapper.getWidth()) / 2f, (VIRTUAL_HEIGHT - wrapper.getHeight()) / 2f);
        ...
        loginBtn.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                String response = LoginMenuController.login(username, password, stay);showMessage(response);
                if (response.equals("Logged in successfully")) {
                     game.setScreen(new MainMenu(game));App.setCurrentMenu(Menu.MAIN_MENU);}
            }
        });
    ...
    private void openForgotPasswordModal() {
        wrapper.setVisible(false);Dialog dialog = new Dialog("Forgot Password", skin);
        ...
        dialog.getButtonTable().add(submitBtn);final int[] step = {1};
            ...
            public void clicked(InputEvent event, float x, float y) {
                if (step[0] == 1) {
                    ...
                    if (response.equals("User does not exist") || response.equals("Incorrect email")) {
                        dialog.hide();showMessage(response);} else {
                        step[0] = 2;content.clearChildren();
                    ...
                    if (response.equals("Please enter your new password")) {step[0] = 3;
