...
public class StartupHelper {
	...
	private static final String LINUX_JRE_ERR_MSG =
        "A Java installation could not be found. If you are distributing this app with a bundled JRE," +
            " be sure to set the environment variable '__GL_THREADED_OPTIMIZATIONS' to '0'!";
	private static final String CHILD_LOOP_ERR_MSG =
        "The current JVM process is a spawned child JVM process, but StartupHelper " +
            "has attempted to spawn another child JVM process! This is a broken state, and should not normally happen!"
            + " Your game may crash or not function properly!";
	...
	public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
		...
		if (!isMac) {
			...
			if ("0".equals(System.getenv("__GL_THREADED_OPTIMIZATIONS"))) return false;} else {
			...
			long currentThread = JNI.invokePPP(nsThread, ObjCRuntime.sel_getUid("currentThread"),
                objcMsgSend);
		...
		if ("true".equals(System.getProperty(JVM_RESTARTED_ARG))) {
			System.err.println(CHILD_LOOP_ERR_MSG);return false;}
		List<String> jvmArgs = new ArrayList<>();
		String javaExecPath = System.getProperty("java.home") + "/bin/java";
		if (!(new File(javaExecPath).exists())) {System.err.println(getJreErrMsg(isMac));return false;}
